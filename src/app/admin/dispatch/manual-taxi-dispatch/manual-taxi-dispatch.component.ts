...
export class ManualTaxiDispatchComponent implements OnInit {
  ...
  customAutocomplete: boolean = AppSettings.googleAutocomplete;
  pickupLocations: any[] = [];
  pickupLocationBuffer: any[] = [];
  pickupLoading = false;
  pickupInput$ = new Subject<string>();

  dropLocations: any[] = [];
  dropLocationBuffer: any[] = [];
  dropLoading = false;
  dropInput$ = new Subject<string>();
  /* ********************** */
  constructor(
  ...
  initializeDispatchForm() {
    this.dispatchForm = new FormGroup({
      ...
      assignType: new FormControl(null),
  ...
  dataFetchWhileReload() {
    ...
    if (id) {
      this.apiservice
        .CommonGetApi("services/request?id=" + id)
        .subscribe((res) => {
  ...
  getAvailableVehicles() {
    ...
    this.apiservice.CommonGetApi(queryParams).subscribe({
      next: (res) => {
        ...
        const selectedVehicle = this.selectVehicles.find(
          (vehicle) => vehicle.vehicleId === this.dispatchForm.value.vehicleId
        );
        if (selectedVehicle) {
    ...
    this.apiservice
      ...
      .subscribe({
        next: (res) => {
          ...
          this.toasterService.success(res.type, res.data.message);
          this.TripInvoiceDialogClose = this.dialogService.open(
            this.TripInvoiceDialog,
            {
              autoFocus: false,
  ...
  generateTimes(startHour: number, startMinute: number): string[] {
    ...
    while (hour < 24) {
      while (minute < 60) {
        const time = `${this.padNumber(hour)}:${this.padNumber(minute)} ${
          hour < 12 ? "AM" : "PM"
        }`;
        times.push(time);
  ...
  fetchMore() {
    ...
    this.getDriver(this.currentPage, this.currentTerm).subscribe({
      next: (res) => {
        console.log(res);
        res.data.partner = res.data.partner.map((item) => ({
          ...item,
          fullName: `${item.fname} ${item.lname}`,
        }));
        this.driverBuffer = this.driverBuffer.concat(res.data.partner);
        this.currentPage++;
        this.loading = false;
      },
      error: (err) => {
    ...
    this.input$
      .pipe(
        ...
        switchMap((term) => {
          ...
          return this.getDriver(this.currentPage, term);
        })
      )
      .subscribe({
        next: (res) => {
          ...
          this.driverBuffer = res.data.partner;
          this.currentPage++;
          console.log(this.driverBuffer);
        },
        error: (err) => {
          console.error(err);
          this.driverBuffer = [];
  ...
  pickupOnSearch() {
    this.pickupInput$
      .pipe(
        ...
        distinctUntilChanged(),
        switchMap((term) => {
          return this.getLocation(term);
        })
      )
      .subscribe({
        next: (res) => {
          this.pickupLocationBuffer = res.data.predictions;
          console.log(this.pickupLocationBuffer);
        },
        error: (err) => {
          console.error(err);
          this.pickupLocationBuffer = [];
        },
      });
  }
  ...
  dropOnSearch() {
    this.dropInput$
      ...
      .subscribe({
        next: (res) => {
          this.dropLocationBuffer = res.data.predictions;
          console.log(this.dropLocationBuffer);
        },
        error: (err) => {
          console.error(err);
          this.dropLocationBuffer = [];
        },
      });
  }

  getDriver(page: number, drivername: string) {
    return this.apiservice.CommonGetApi(
      `common/partner?page=${page}&limit=10&online=0${
        drivername ? "&fname=" + drivername : ""
      }`
    );
  }
  ...
  getLocation(location: string) {
    if (location) {
      return this.apiservice.CommonGetApi(
        `module/services/request/location?${
          location ? `location=${location}` : ""
        }`
      );
    } else {
      return of({ data: { predictions: [] } });
    }
  }
  ...
  setLocation(e, type) {
    console.log(e);
    this.apiservice
      .CommonGetApi(`module/services/request/latlng?placeId=${e.placeId}`)
      .subscribe({
        next: (response) => {
          if (type == "pickup") {
            this.pickupLat = response.data.lat;
            this.pickupLang = response.data.lng;
          } else {
            this.dropLat = response.data.lat;
            this.dropLang = response.data.lng;
          }
          console.log(
            "set custom autocomplete lat lng",
